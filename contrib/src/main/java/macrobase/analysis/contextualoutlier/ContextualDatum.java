...
public class ContextualDatum extends Datum {
    ...
    public ContextualDatum(Datum d, MacroBaseConf conf) throws ConfigurationException {
        ...
        List<String> contextualDiscreteNames = conf.getStringList(ContextualConf.CONTEXTUAL_DISCRETE_ATTRIBUTES, null);
        ...
        List<String> contextualDoubleNames = conf.getStringList(ContextualConf.CONTEXTUAL_DOUBLE_ATTRIBUTES, null);
        if (contextualDoubleNames != null) {
            ...
            for (String attr : contextualDoubleNames) {
                int pos = attributes.indexOf(attr);
                contextualDoubleAttributes.setEntry(vecPos, Double.parseDouble(conf.getEncoder().getAttribute(d.attributes().get(pos)).getValue()));
                vecPos += 1;
            }
        }
    }
