...
public class DiskCachingIngester extends DataIngester {
    ...
    private String convertFileName(Integer timeColumn, List<String> attributes, List<String> metrics, String baseQuery) {
        String timeColumnName = conf.getEncoder().getAttributeName(timeColumn);
        int hashCode = String.format("T-%s::A-%s::M%s::BQ%s", timeColumnName, attributes.toString(), metrics.toString(), baseQuery).replace(" ", "_").hashCode();
        return Integer.toString(hashCode);
    }

    private void writeOutData(List<Datum> data) throws IOException {
        OutputStream outputStream = new SnappyOutputStream(new BufferedOutputStream(new FileOutputStream(fileDir + "/" + convertFileName(timeColumn, attributes, metrics, conf.getString(MacroBaseConf.BASE_QUERY, conf.getString(MacroBaseConf.QUERY_NAME, "cachedQuery"))))), 16384);
        Kryo kryo = new Kryo();
        Output output = new Output(outputStream);
        kryo.writeObject(output, conf.getEncoder());
        final int BATCHSIZE = conf.getInt(MacroBaseConf.DB_CACHE_CHUNK_SIZE, MacroBaseDefaults.DB_CACHE_CHUNK_SIZE);
    ...
    private List<Datum> readInData() throws IOException {
        File f = new File(fileDir + "/" + convertFileName(timeColumn, attributes, metrics, conf.getString(MacroBaseConf.BASE_QUERY, conf.getString(MacroBaseConf.QUERY_NAME, "cachedQuery"))));
        if (!f.exists()) {
